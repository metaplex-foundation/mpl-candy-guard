...
---
## Update:
Candy Guard v0.2.0 updated the serialization logic for the arguments of the `initialize` and `update` instructions. Instead of using a typed struct as the arrgument, these instructions expect a `[u8]` represeting the serialized struct. This is to ensure adding new guards in the future does not affect clients.
