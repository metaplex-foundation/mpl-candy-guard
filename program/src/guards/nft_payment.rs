...
impl Condition for NftPayment {
    fn validate<'info>(
        ...
        let nft_mint = get_account_info(ctx, index + 2)?;
        ...
        let destination = get_account_info(ctx, index + 3)?;
        let destination_ata = get_account_info(ctx, index + 4)?;
        let _atoken_program = get_account_info(ctx, index + 5)?;
        evaluation_context.account_cursor += 3;
